...
CameraParameters setCameraParameterscu(CameraParameters_cu& CP,std::vector<ImagePtr>& Images_,const int reference,float scaleFactor,bool transformP){
   ...
   for(size_t i=0;i<numCameras;++i){
        if(Images_[i]->cameraCpu_->K_.type()!=CV_32F){
            Images_[i]->cameraCpu_->K_.convertTo(Images_[i]->cameraCpu_->K_,CV_32F);
            Images_[i]->cameraCpu_->R_.convertTo(Images_[i]->cameraCpu_->R_,CV_32F);
            Images_[i]->cameraCpu_->T_.convertTo(Images_[i]->cameraCpu_->T_,CV_32F);
            //Images_[i]->cameraCpu_->OptionCenter_.convertTo(Images_[i]->cameraCpu_->OptionCenter_,CV_32F);
        }
        k[i]=Images_[i]->cameraCpu_->K_;
        R[i]=Images_[i]->cameraCpu_->R_; 
        T[i]=Images_[i]->cameraCpu_->T_;
...
void PreconditioningGPUdata(std::vector<ImagePtr>&Images_,const int reference){
    ...
    {
        Images_[i]->ImgDataGray.convertTo(img_grayscale_float[i], CV_32FC1); // or CV_32F works (too)
        Images_[i]->ImgDataGray.convertTo(img_grayscale_uint[i], CV_16UC1); // or CV_32F works (too)
        if(algParams->colorProcessing) {
            ...
            Images_[i]->ImgDataRGB.convertTo (img_color_float[i], CV_32FC3); // or CV_32F works (too)
...
void saveImgDepth(ImagePtr referenceImg,GlobalState& globalState ){
    ...
    // std::string fileName=
    cv::imwrite("output"+std::to_string(referenceImg->imgId_)+".png", normalizedImage);
    // cv::Mat normalizedImage;
